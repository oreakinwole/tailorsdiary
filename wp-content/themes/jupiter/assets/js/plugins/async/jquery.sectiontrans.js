  ...
  Plugin.prototype = {
    ...
    toggle: function (reveal) {
      ...
      if (reveal) {
        ...
        // Only do this for intro with fade effect
        if ( $( this.element ).hasClass( 'intro-effect-fade' ) ) {
          $(this.element).animate({top: '-100%'}, 3000);
        }
        ...
        // Only do this for intro with fade effect
        if ( $( this.element ).hasClass( 'intro-effect-fade' ) ) {
          $(this.element).removeClass('mk-intro-triggered').css('top','0');
        }
